...
async def set_not_afk(event):
    ...
    if (("afk" not in current_message) or ("#afk" not in current_message)) and (
        ...
        shite = await event.client.send_message(
            event.chat_id,
            "`Sono tornato stronzo! Non sono più afk.\nEro afk per " + endtime + "`",
        )
        AFK_.USERAFK_ON = {}
        AFK_.afk_time = None
...
async def on_afk(event):  # sourcery no-metrics
    ...
    if AFK_.USERAFK_ON and not (await event.get_sender()).bot:
        ...
        if AFK_.afk_type == "media":
            if AFK_.reason:
                message_to_reply = (f"〜 STO DORMENDO💤 〜\n\nStronzi • Non online\nOffline da • {endtime}\nMotivo • {AFK_.reason}")
            else:
            ...
            if AFK_.msg_link and AFK_.reason:
                ...
                message_to_reply = f"〜 STO DORMENDO💤 〜\n\nStronzi • Non online\nOffline da • {endtime}\nMotivo • Not Mentioned ( ಠ ʖ̯ ಠ)"
            if event.chat_id:
                ss = await event.client.send_file(
                    BOTLOG_CHATID,
                    "https://telegra.ph/file/a8e2fc5b6a2025a77b6ab.mp4",
                    caption=f"{message_to_reply}",
                )
                msg = await event.reply(ss)
                await ss.delete()
...
async def _(event):
    ...
    if not AFK_.USERAFK_ON:
        ...
        if AFK_.reason:
            await edit_delete(
                event, f"`Io sarò afk stronzi!\nperché = ` {AFK_.reason}", 5
            )
        else:
            await edit_delete(event, "`Io sarò afk stronzi! `", 5)
        if BOTLOG:
            if AFK_.reason:
                await event.client.send_message(
                    BOTLOG_CHATID,
